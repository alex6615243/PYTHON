...
def trace_direction(current_device, direction='downstream'):
    ...
    while True:
        ...
        # 儲存邊，方向依據
        if direction == 'downstream':
            path_edges.append((from_dev, to_dev, circuit))
...
def draw_full_graph(up_edges, down_edges, center_device):
    ...
    # 添加邊及其顏色
    edge_colors = []
    for from_dev, to_dev, circuit in up_edges + down_edges:
        G.add_edge(from_dev, to_dev, circuit=circuit)
        edge_colors.append(circuit_colors.get(circuit, 'black'))  # 默認為黑色
    ...
    # 標註各個邊的 Circuit 和 Cable No.
    for from_dev, to_dev, circuit in up_edges + down_edges:
        label = f"{circuit}\n{df[(df['From'] == from_dev) & (df['TO'] == to_dev)]['Cable No.'].values[0]}"
        nx.draw_networkx_edge_labels(G, pos, edge_labels={(from_dev, to_dev): label}, font_size=8)

    # 設置圖例
    legend_handles = [mpatches.Patch(color=color, label=circuit) for circuit, color in circuit_colors.items()]
    plt.legend(handles=legend_handles, loc='lower right', fontsize=10)

    # 標題
    plt.title(f"設備 {center_device.upper()} 的上下游電路圖", fontsize=14)
    plt.show()
