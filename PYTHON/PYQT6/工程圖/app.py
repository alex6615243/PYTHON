...
if not edited_df.equals(st.session_state.tasks):
    # 🛡️ 增加保護鎖：如果編輯後的表格是空的，但不該是空的，攔截同步（防止意外全刪）
    if edited_df.empty and not st.session_state.tasks.empty:
        st.warning("⚠️ 檢測到清空表格操作，自動同步已攔截。如欲清空請手動刪除每一列或建立備份。")
    else:
        try:
            # 1. 濾掉未填寫完整的列
            ...
            for _, row in clean_df.iterrows():
                ...
                # 區域/廠商防呆
                final_reg = val_reg if val_reg in st.session_state.regions else default_reg
                final_sub = val_sub if val_sub in st.session_state.subcontractors else default_sub
            ...
            # 2. 執行資料庫同步 (僅當有內容或確定要變更時)
            # 我們這裡先刪除再新增，這就是為什麼保護鎖很重要的原因
